...
## [2.0.1] - 2026-06-18
### Fixed
- **`tiers` collection**: `GET /tiers` trả về rỗng dù `GET /tiers/:id` vẫn lấy được tier bình thường.
  - **Nguyên nhân**: `001_init-tiers-and-admin.ts` insert tier qua MongoDB driver thuần (`db.collection('tiers').updateOne(...)`), bypass Mongoose nên schema default (`isActive: true`) và field required (`noMonth`) không được ghi vào document thật trong DB. `TierController.listTiers` filter theo `{ isActive: true }` → không match document nào thiếu field này → trả rỗng. `getTierById` không filter theo `isActive` nên vẫn lấy được.
  - **Fix**:
    - Thêm migration `002_backfill-tier-fields.ts` để backfill `isActive: true` + `noMonth: 1` cho 3 tier đã tồn tại trong DB (đã chạy trên Atlas).
    - Sửa `001_init-tiers-and-admin.ts` để seed đủ field (`isActive`, `noMonth`) ngay từ đầu cho môi trường mới.

### Script Data
- Chạy `npm run db:migrate` để áp dụng `002_backfill-tier-fields.ts` nếu DB của bạn cũng thiếu field này (kiểm tra bằng `npm run db:status`).
...
- **`historicalcontexts` collection**: Historical periods/events.
  - Fields: `createdBy`, `name`, `description`, `era` (enum), `year`, `isBC`, `location`, `imageUrl`, `videoUrl`, `characterIds[]` (replaces ContextCharacterMapping join table).
